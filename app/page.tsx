...
type Sheet = {
  ...
  sourcePreviewUrl?: string; sourceFileName?: string; sourceMimeType?: string;
};
...
export default function Home() {
  ...
  async function publish(sheet:Sheet){const persistable=Object.fromEntries(Object.entries(sheet).filter(([key])=>key!=="sourcePreviewUrl")) as Sheet;setUploads(current=>[sheet,...current]);if(!isStaticSite)try{const response=await fetch("/api/sheets",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify(persistable)});const payload=await response.json() as {persisted?:boolean};if(payload.persisted){localStorage.removeItem("bandproject-uploads");return}}catch{/* retain browser fallback below */}const current=JSON.parse(localStorage.getItem("bandproject-uploads")||"[]") as Sheet[];localStorage.setItem("bandproject-uploads",JSON.stringify([persistable,...current.filter(item=>item.id!==sheet.id)]))}
