...
function AnalysisV2() {
  const [stage,setStage]=useState<"upload"|"analyzing"|"results"|"transcription">("upload"); const [progress,setProgress]=useState(0); const [target,setTarget]=useState("Guitar"); const [saved,setSaved]=useState(false); const [error,setError]=useState(""); const [editing,setEditing]=useState(false); const [result,setResult]=useState<AudioAnalysisResult|null>(null); const [serverScore,setServerScore]=useState<MusicProcessingResult|null>(null); const [audioFile,setAudioFile]=useState<File|null>(null); const [audioUrl,setAudioUrl]=useState(""); const [playing,setPlaying]=useState(false); const [playhead,setPlayhead]=useState(0); const [transcribing,setTranscribing]=useState(false); const input=useRef<HTMLInputElement>(null); const player=useRef<HTMLAudioElement>(null); const audioUrlRef=useRef("");
  function targetForApi(){return target.toLowerCase().replace(" ","-")}
  async function requestTranscription(file:File):Promise<MusicProcessingResult>{if(isStaticSite)return {provider:"GitHub Pages browser mode",mode:"fallback",warnings:["The static GitHub Pages version analyzes audio locally. No transcription notes or MusicXML are available here."]};const body=new FormData();body.append("file",file);body.append("target_instrument",targetForApi());body.append("source_type","unknown");return fetch("/api/transcribe",{method:"POST",body}).then(async response=>{const payload=await response.json() as MusicProcessingResult&{error?:string};if(!response.ok)throw new Error(payload.error||"Transcription provider failed.");return payload})}
