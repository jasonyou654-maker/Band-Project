...
function readStoredIds(key: string, legacyKey?: string): number[] {
  if (typeof window === "undefined") return [];
  try {
    const current = localStorage.getItem(key);
    const legacy = current ? null : legacyKey ? localStorage.getItem(legacyKey) : null;
    const value: unknown = JSON.parse(current || legacy || "[]");
...
function ScoreFeatureSummary({ musicXml }: { musicXml: string }) {
  ...
  const features = candidates.filter(([, value]) => value > 0);
...
function SourcePreview({ sheet, zoom }: { sheet: Sheet; zoom: number }) {
  ...
  return <div className="source-file-preview image-preview" style={{ "--score-zoom": zoom / 100 } as React.CSSProperties}><img src={sheet.sourcePreviewUrl} alt={`${sheet.title} 原始乐谱`} /></div>;
}
...
function Logo({ onClick }: { onClick: () => void }) { return <button className="logo" onClick={onClick}><span className="logo-mark">S17</span><span>Studio17</span></button>; }
...
function Explore({ openSheet, library, likedIds, savedIds, toggleLike, toggleSave }: { openSheet: (s: Sheet) => void; library: Sheet[]; likedIds: number[]; savedIds: number[]; toggleLike: (sheet: Sheet) => void; toggleSave: (sheet: Sheet) => void }) {
  ...
  return <main>
    <section className="explore-hero"><div className="hero-note">STUDIO17 · YOUR MUSIC LIBRARY</div><h1>Play something<br/><em>worth sharing.</em></h1><p>Discover arrangements, build your library, and share scores with clear credits.</p>
...
function SheetDetail({ sheet, back, liked, saved, toggleLike, toggleSave }: { sheet: Sheet; back: () => void; liked: boolean; saved: boolean; toggleLike: () => void; toggleSave: () => void }) {
  ...
  async function share(){ await navigator.clipboard?.writeText(`${sheet.title} — ${sheet.artist} on Studio17`); setShared(true); setTimeout(() => setShared(false), 1800); }
  useEffect(() => {
...
function Upload({ done, publish }: { done: () => void; publish: (sheet: Sheet) => Promise<void> }) {
  ...
  if (submitted) return <main className="success-page"><div className="success-icon"><Icon name="check" size={32}/></div><span className="kicker">UPLOAD COMPLETE</span><h1>Your arrangement is in.</h1><p>It&apos;s now part of the Studio17 community. You can edit details any time from your profile.</p><button className="primary" onClick={done}>Explore the community <Icon name="arrow"/></button></main>;
...
function AnalysisV2Legacy() {
  ...
  if(stage==="upload")return <main className="ai-page"><section className="ai-hero"><span className="ai-label"><i/> STUDIO17 AUDIO LAB</span><h1>Hear more in<br/>every song.</h1><p>Analysis runs on the audio itself in your browser: onset timing, tonal energy, chord candidates, and dynamics—not a filename lookup.</p></section><section className="audio-upload"><button onClick={()=>input.current?.click()}><input ref={input} type="file" accept="audio/*" onChange={e=>analyze(e.target.files?.[0])}/><span><Icon name="music" size={24}/></span><h3>Drop an audio file here</h3><p>or click to browse · MP3, WAV, M4A up to 50 MB</p></button>{error&&<div className="analysis-error">{error}</div>}<div className="privacy"><Icon name="check" size={15}/>{isStaticSite?"Audio is decoded locally in this static preview.":"Audio is sent to the Studio17 processing service only to create your transcription draft."}</div></section><section className="ai-benefits"><article><b>01</b><h3>Measure the pulse</h3><p>Onset-energy autocorrelation estimates tempo from recurring attacks.</p></article><article><b>02</b><h3>Map the harmony</h3><p>Pitch-class energy is compared across major, minor, and chord templates.</p></article><article><b>03</b><h3>Make a draft</h3><p>Transcription starts from the detected key, tempo, and progression for review.</p></article></section></main>;
...
export default function Home() {
  const [view, setView] = useState<View>("explore"); const [selected, setSelected] = useState(sheets[0]); const [uploads,setUploads]=useState<Sheet[]>([]); const [likedIds,setLikedIds]=useState<number[]>(()=>readStoredIds("studio17-liked", "bandproject-liked")); const [savedIds,setSavedIds]=useState<number[]>(()=>readStoredIds("studio17-saved", "bandproject-saved")); const [activity,setActivity]=useState<Activity[]>([]); const [toast,setToast]=useState("");
  const library=[...uploads,...sheets];
  useEffect(()=>{queueMicrotask(async()=>{let localUploads:Sheet[]=[];try{const stored=localStorage.getItem("studio17-uploads")||localStorage.getItem("bandproject-uploads");if(stored){localUploads=(JSON.parse(stored) as Sheet[]).filter(isTrustedScore).map(restoreSourcePreview);localStorage.setItem("studio17-uploads",JSON.stringify(localUploads.map(sheet=>{const persistable={...sheet};delete persistable.sourcePreviewUrl;return persistable})))}}catch{/* ignore corrupt fallback data */}if(!isStaticSite)try{const response=await fetch("/api/sheets",{cache:"no-store"});const payload=await response.json() as {sheets?:Sheet[]};if(payload.sheets?.length){const localById=new Map(localUploads.map(sheet=>[sheet.id,sheet]));setUploads(payload.sheets.filter(isTrustedScore).map(sheet=>restoreSourcePreview({...sheet,likes:0,saves:0,downloads:0,...localById.get(sheet.id)})));return}}catch{/* D1 is optional in local development */}setUploads(localUploads)})},[]);
  ...
  function toggleLike(sheet:Sheet){const added=!likedIds.includes(sheet.id);const next=added?[sheet.id,...likedIds]:likedIds.filter(id=>id!==sheet.id);setLikedIds(next);localStorage.setItem("studio17-liked",JSON.stringify(next));record(added?`已点赞「${sheet.title}」`:`已取消点赞「${sheet.title}」`)}
