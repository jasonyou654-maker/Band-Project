...
export default function Home() {
  const [view, setView] = useState<View>("explore"); const [selected, setSelected] = useState(sheets[0]); const [uploads,setUploads]=useState<Sheet[]>([]); const [likedIds,setLikedIds]=useState<number[]>([]); const [savedIds,setSavedIds]=useState<number[]>([]); const [activity,setActivity]=useState<Activity[]>([]); const [toast,setToast]=useState("");
  ...
  useEffect(()=>{try{setLikedIds(JSON.parse(localStorage.getItem("bandproject-liked")||"[]"));setSavedIds(JSON.parse(localStorage.getItem("bandproject-saved")||"[]"))}catch{/* ignore invalid local preferences */}},[]);
