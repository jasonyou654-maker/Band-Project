...
const worker = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/_vinext/image") {
      ...
      return handleImageOptimization(request as Parameters<typeof handleImageOptimization>[0], {
...
async function storeUploadedSource(request: { formData(): Promise<FormData> }, bucket: R2Bucket, kind: string) {
  try {
